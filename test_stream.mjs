...
page.on('console', msg => {
  const text = msg.text();
  if (text.includes('mlx_stream') || text.includes('import') || text.includes('LinkError') || text.includes('not a function')) {
    errors.push(text);
...
if (errors.length) { console.log('\nErrors during init:'); errors.forEach(e => console.log(e)); }
