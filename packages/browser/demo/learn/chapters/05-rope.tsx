  ...
  return (
    <ChapterFrame learning={learning}>
      ...
      <p>
        ...
        <strong>permutation-invariant</strong>. If you shuffle the input
        tokens, the attention output for each token shuffles right along with
        them, but the relationships the attention layer computes don't
        change. Without help, a transformer literally cannot tell{" "}
        <em>"the cat sat on the mat"</em> from <em>"mat the on sat cat the"</em>.
        Something has to inject the order back in.
      </p>

      <h2>Two earlier ideas</h2>
      <ul>
        <li>
          <strong>Learned positional embeddings.</strong> Early GPT, BERT, and
          friends gave every position 0..N-1 its own learned vector and
          <em>added</em> it to the token embedding. Simple, but the model
          can't extrapolate to positions it never saw during training, and
          you have to pick a max length up front.
        </li>
        <li>
          <strong>Sinusoidal embeddings.</strong> The original 2017
          transformer used fixed sinusoids of different frequencies, also
          added to the token embedding. Extrapolation is in principle
          possible, but in practice quality degrades past the training
          window.
        </li>
      </ul>
      ...
      <p>
        <strong>Rotary Positional Embedding</strong> (RoPE) leaves the
        embeddings alone and instead bakes the position directly into the
        attention math. The trick is to <strong>rotate</strong> the query
        and key vectors by an angle that depends on the token's position,
        <em>before</em> the dot product is computed.
      </p>
      <p>
        Concretely, RoPE pairs up adjacent feature dimensions —{" "}
        <code>(x_0, x_1)</code>, <code>(x_2, x_3)</code>, … — and treats
        each pair as a point in a 2D plane. For pair index <code>i</code>{" "}
        and head dimension <code>d</code>, the frequency is:
      </p>
      <pre>
        <code>{`theta_i = base^(-2i / d)`}</code>
      </pre>
      <p>
      ...
      <pre>
        <code>{`(x_{2i}',   x_{2i+1}') = ( x_{2i} cos(m·theta_i) - x_{2i+1} sin(m·theta_i),
                          x_{2i} sin(m·theta_i) + x_{2i+1} cos(m·theta_i) )`}</code>
      </pre>
      <p>
        Rotation is <strong>unitary</strong> — it preserves vector norms —
        so RoPE doesn't change how big Q and K are, only where they point.
        The position information rides on the angle, not the magnitude.
      </p>

      <h2>Wide range of frequencies</h2>
      <p>
        Low-index pairs (small <code>i</code>) get the highest frequencies
        and rotate fast — one full turn every few tokens. High-index pairs
        get vanishingly small frequencies and barely move even over tens of
        thousands of tokens. The intuition: high-frequency pairs encode
        <strong> fine local position</strong> ("am I 3 tokens from my
        neighbour?"), low-frequency pairs encode{" "}
        <strong>coarse global position</strong> ("am I in the early part of
        the document or the late part?").
      </p>
      <p>
        Qwen3.5 uses a head dimension of <code>{HEAD_DIM}</code>, a partial
        rotary factor of <code>{PARTIAL_ROTARY_FACTOR}</code> (so only the
        first <code>{ROPE_DIMS}</code> features per head are rotated — the
        rest pass through untouched), and an unusually large RoPE base of{" "}
        <code>{ROPE_THETA.toExponential(0)}</code>. The large base stretches
        the frequency spectrum so the lowest-frequency pairs barely rotate
        at all over the model's <code>{MAX_POSITION.toLocaleString()}</code>
        -token context window — a key ingredient for long-context
        extrapolation.
      </p>

      <h2>The relative-position property</h2>
      <p>
        Here is the part that makes RoPE click. If you take the dot product
        of <code>RoPE(q, m)</code> with <code>RoPE(k, n)</code>, the answer
        depends <em>only on the difference</em> <code>m - n</code>, not on{" "}
        <code>m</code> and <code>n</code> separately. The model never has
        to learn what "position 1,427" means — it only has to learn how
        attention should behave at <em>relative</em> offsets, which
        generalises naturally to lengths it has never seen.
      </p>
      <p>
        The third panel on the right makes this concrete. Holding the query
        position fixed at <code>m = {DOT_QUERY_POS}</code> and varying the
        key position <code>n</code>, the rotated dot product peaks sharply
        at <code>n - m = 0</code> and decays as you move further apart.
        That decay is exactly the inductive bias that makes attention "want"
        nearby tokens more than distant ones, with no per-position
        parameters at all.
      </p>

      <p className="mt-6 text-muted-foreground">
        This chapter visualises RoPE's math directly — no model inference
        needed. Want to see how this plays out on real attention scores?
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-md border border-dashed border-muted-foreground/40 bg-muted/40 px-3 py-2 text-xs text-muted-foreground">
        This chapter visualises RoPE's math directly — no model inference
        needed. The numbers below come from Qwen3.5-0.8B's config:
        head_dim={HEAD_DIM}, rotated dims={ROPE_DIMS} ({NUM_PAIRS} pairs),
        rope_theta={ROPE_THETA.toExponential(0)}.
      </div>
      ...
      <div className="rounded-md bg-muted/40 px-3 py-2 font-mono text-[11px] text-muted-foreground">
        Pair {selectedPair} · freq = {formatFreq(selectedFreq)} rad/token ·
        period ≈ {formatPeriod(selectedPeriod)} tokens
      </div>
      ...
      <SliderRow
        ...
        step={1}
        onChange={(v) => setRotationPos(Math.round(v))}
        format={(v) => `m = ${v}`}
      />
  ...
  return (
    <div className="rounded-md border border-border bg-background p-3">
      ...
      >
        {ticks.map((t, idx) => {
          const y =
            padTop + innerH - ((t - logMin) / logSpan) * innerH;
          return (
            <g key={idx}>
              <line
                x1={padLeft}
          ...
          return (
            <rect
              ...
              className={
                isSelected
                  ? "fill-primary"
                  : "fill-foreground/40 hover:fill-foreground/70"
              }
              onClick={() => onSelect(i)}
      ...
      <div className="pt-1 text-[11px] text-muted-foreground">
        Each bar is one dimension pair. Click or use the slider below to
        select a pair and update the rotation panels.
      </div>
    </div>
  );
}
...
function RotationPanel({
  ...
  // Build a trail arc from 0 to `wrapped` so the user can see the swept
  // angle for the current position.
  function arcPath(): string {
    if (wrapped < 1e-4) return "";
    const startX = cx + r;
    const startY = cy;
    const endX = handX;
    const endY = handY;
    const largeArc = wrapped > Math.PI ? 1 : 0;
    // SVG arcs use clockwise as sweep=1; our angle is CCW in math terms,
    // so flip the y-axis by using sweep=0.
  ...
  return (
    <div className="rounded-md border border-border bg-background p-3">
      ...
      >
        ...
        <circle cx={cx + r} cy={cy} r={2} fill="currentColor" fillOpacity={0.3} />
      </svg>
...
function DotProductChart({
  ...
  return (
    <div className="rounded-md border border-border bg-background p-3">
      ...
      <div className="pt-1 text-[11px] text-muted-foreground">
        With a constant query and key, the rotated dot product depends only
        on (n − m). The peak at n = m and smooth decay either side is the
