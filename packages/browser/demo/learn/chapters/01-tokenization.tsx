...
const DEFAULT_PROMPT =
  "The quick brown fox jumps over the lazy dog. Antidisestablishmentarianism. 你好";
const CODE_EXAMPLE =
  ...
  return (
    <ChapterFrame learning={learning}>
      ...
      <p>
        ...
        <strong>tokens</strong>, and the rule that decides where the cuts go
        is called the <em>tokenizer</em>. Tokens are not characters, and they
        are not words — they are something in between, chosen by an algorithm
        called <strong>Byte-Pair Encoding (BPE)</strong>. The widget on the
        right runs Qwen3's exact tokenizer in your browser. Type, watch the
        chips refresh.
      </p>

      <h2>Why sub-words?</h2>
      <p>
        Tokenizers sit in an awkward middle. If we made every <em>word</em> a
        token, the vocabulary would need millions of entries to cover the long
        tail of names, typos and made-up words — and the embedding matrix
        would balloon along with it. If we went the other way and made every{" "}
        <em>character</em> a token, the model would have to chew through
        sequences five-to-ten times longer than the same text in words, and
        every layer's cost is at least linear in sequence length.
      </p>
      <p>
        BPE finds a middle path: start from raw bytes (or characters), then
        repeatedly merge the most frequent adjacent pair into a new symbol
        until the vocabulary is the size you want. Common words like{" "}
        <code>the</code> end up as a single token; rarer words like{" "}
        <code>Antidisestablishmentarianism</code> decompose into shorter
        familiar pieces. The tokenizer doesn't know what a word means — it
        only knows what byte sequences are statistically frequent in the
        training corpus.
      </p>

      <h2>What Qwen3 does specifically</h2>
      <p>
        Qwen3 ships with a BPE tokenizer of roughly <strong>152,000</strong>{" "}
        entries. A few quirks are worth knowing:
      </p>
      <ul>
        <li>
          <strong>Spaces live inside tokens.</strong> The BPE algorithm treats
          a space-prefixed word as a different symbol from the bare word, so{" "}
          <code>"cat"</code> at the start of a sentence and <code>" cat"</code>{" "}
          mid-sentence are usually two different tokens. The chips on the
          right mark a leading space with a small dot (<code>·</code>) so you
          can see this without it being invisible.
        </li>
        <li>
          <strong>Special tokens mark structure.</strong> Sequences like{" "}
          <code>&lt;|im_start|&gt;</code> and <code>&lt;|im_end|&gt;</code>{" "}
          aren't text the model generates — they're chat-turn boundaries used
          by Qwen3's chat template. The widget renders any token whose
          decoded text contains <code>&lt;|…|&gt;</code> with a dashed border
          so you can spot them.
        </li>
        <li>
          <strong>Multilingual coverage isn't uniform.</strong> Latin-script
          words are usually one or two tokens; CJK characters often land as
          one token each but unusual scripts can decompose into byte-level
          fragments. Try the multilingual example to see the difference.
        </li>
      </ul>

      <h2>Why this matters</h2>
      <p>
        Every downstream cost a language model has is paid <em>per token</em>:
        API billing, context-window limits, KV-cache memory, and the
        wall-clock latency of generation. A prompt that looks short to a
        human can be long to the model if it uses rare punctuation, code, or
        languages the tokenizer didn't see much of in training. The{" "}
        <strong>characters-per-token ratio</strong> in the stats footer is
        the simplest way to feel this: prose in well-supported languages
        usually lands near 4 chars/token, code closer to 2-3, and an
        emoji-heavy or rare-script string can drop below 1.
      </p>
      <p>
        Different model families use different tokenizers, and the same
        string can come out radically different lengths. That's why
      ...
      <p className="mt-6 text-muted-foreground">
        ...
        token ids becomes a high-dimensional vector that the model can
  ...
  const runTokenize = React.useCallback(
    async (text: string) => {
      ...
      if (!worker) {
        ...
        console.log(
          "[tokenizer-demo] using heuristic fallback (model not loaded)",
          { promptLength: text.length, pieces: fallback.length },
        );
        return;
      }
      ...
      try {
        ...
        if (isAbortError(err)) {
          ...
          console.error(
            "[tokenizer-demo] tokenize failed; using heuristic fallback",
            err,
          );
          const fallback = fallbackHeuristicTokens(text);
          setTokens(fallback);
  ...
  return (
    <div className="space-y-4">
      <div className="space-y-2">
        ...
        <div className="flex flex-wrap items-center gap-2 text-xs">
          <Button
            variant="outline"
            size="sm"
            onClick={() => setPrompt(CODE_EXAMPLE)}
          >
            Try a code snippet
          </Button>
          <Button
            variant="outline"
            size="sm"
            onClick={() => setPrompt(MULTILINGUAL_EXAMPLE)}
          >
            Try multilingual
          </Button>
          <span className="text-muted-foreground">
            {running ? "Tokenizing…" : "Live encode on every keystroke."}
          </span>
        </div>
      </div>
      ...
      {status?.kind === "fallback-no-worker" ? (
        ...
        >
          Showing a whitespace split — load the model first to see the real
          Qwen3 BPE tokens.
        </div>
      ) : null}
      {status?.kind === "fallback-error" ? (
        ...
        >
          <strong>Tokenize failed.</strong> Showing a whitespace fallback
      ...
      <div
        aria-live="polite"
        className="rounded-md bg-muted/40 px-3 py-2 font-mono text-xs text-muted-foreground"
      >
