...
async function initAndRun(wasmUrl: string) {
  ...
  // ---- Tests ----
  const tests: { name: string; run: () => unknown }[] = [
    {
      name: 'create float32',
      run() {
    ...
    // — the brief explicitly authorizes the fallback.
    {
      ...
      run() {
        ...
        // Numerical parity gate. Pure element-wise + matmul chain compiled
        // by `mlx::core::compile` should be byte-identical (or within 1
        // ulp) to the eager path on f32 — the compile pass doesn't reorder
        // accumulators inside matmul or change the sigmoid expression.
        // Use 1e-5 to leave room for any reduction-tree rearrangement
        // inside the WGSL kernels for fused element-wise ops, which
        // shouldn't apply here but cost nothing to allow.
        if (!(maxAbsErr <= 1e-5)) {
          throw new Error(
            `Phase 6b numerical parity FAILED: maxAbsErr=${maxAbsErr} > 1e-5 ` +
