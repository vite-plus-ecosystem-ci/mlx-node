//! Gated session-reset regression test for Qwen3.5 MoE on the PAGED
//! adapter.
//!
...
//! the lfm2 fix in 47d8dc53; qwen3_5_moe shares the identical paged
//! adapter lifecycle.)
//!
//! qwen3_5_moe paged is OPT-IN (default OFF), so the test clones the
...
//! built before exercising the reset. The existing
//! `qwen3_5_moe_session.rs` tests load WITHOUT forcing paged (so they
//! run FLAT and do not exercise this path) — hence a dedicated file.
//!
//! Gated on `MLX_TEST_MODEL_PATH`. Run with:
//!
//! ```shell
//! MLX_TEST_MODEL_PATH=/Volumes/P4510/models/qwen3.6-35b-a3b-mxfp8-mtp \
//!     PATH=/usr/bin:$PATH SDKROOT=$(xcrun --show-sdk-path) \
//!     cargo test -p mlx-core --test qwen3_5_moe_reset_purge -- --ignored --nocapture
//! ```
//!
...
/// injected. Mirrors `qwen3_5_moe_paged_vs_flat_parity.rs::clone_model_dir`.
...
#[ignore = "needs MLX_TEST_MODEL_PATH pointing to a real Qwen3.5 MoE checkpoint"]
async fn qwen3_5_moe_session_reset_purges_prefix_cache_cold_prefill() {
    ...
    // Force the paged adapter ON (qwen3_5_moe is opt-in / default OFF).
    let paged_dir = match clone_model_dir_paged(&src, "qwen35-moe") {
        Ok(p) => p,
    ...
    let model = Qwen3_5MoeModel::load(paged_dir.to_string_lossy().to_string())
        .await
        .expect("failed to load paged Qwen3.5 MoE model");
    assert!(
    ...
    // prompt_len - 1` (model.rs:1938) and `find_longest_cache_hit` only
