...
//!
//! ```shell
//! MLX_TEST_MODEL_PATH=./.cache/models/qwen3_5-moe-mlx-bf16 \
//!     cargo test -p mlx-core --test qwen3_5_moe_paged_vs_flat_parity \
//!     -- --ignored --nocapture
//! ```
...
async fn qwen3_5_moe_paged_vs_flat_greedy_token_parity() {
    let Some(src) = resolve_source_model() else {
        return;
    };

    let flat_dir = match clone_model_dir(&src, "qwen35moe-flat", false) {
        Ok(p) => p,
        Err(e) => panic!("failed to clone model dir for flat path: {e}"),
    };
    let paged_dir = match clone_model_dir(&src, "qwen35moe-paged", true) {
        Ok(p) => p,
    ...
    let flat_model = Qwen3_5MoeModel::load(flat_dir.to_string_lossy().to_string())
        .await
        .expect("failed to load flat-path Qwen3.5 MoE model");
    ...
    eprintln!(
        "Qwen3.5 MoE greedy parity: all {} prompts matched",
        prompts.len()
    );
}
